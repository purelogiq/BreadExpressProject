## Notes to graders ##
This site is responsive but not designed for small mobile screens. About an 1000px of width is needed on at least an 8in wide screen.
An assumption is made that the bakery doesn't have hundreds of active items at once and so the item listing on the store page is not paginated.
Please clear browser cache and cookies for localhost before use (or open in new private window/incognito mode). FIrefox is the perffered browser.
Users can view a list of items by going to /items and details with the item show page BUT the site is designed for them to look at items on the /shop page. The /items page is there for the administrator and to pass cucumber tests.
I believe I did the extra credit for toggling active/inactive items.
You may like the errors page...



## 67-272: Bread Express (imadueme@andrew.cmu.edu) ##
